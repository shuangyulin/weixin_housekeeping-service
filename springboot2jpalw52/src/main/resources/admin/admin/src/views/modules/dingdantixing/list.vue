		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 0 20px","borderRadius":"0px","textAlign":"left","background":"none","display":"block","width":"100%","fontSize":"inherit","order":"2"}' >
					...
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="服务状态" prop="fuwuzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">服务状态</label>
						<el-select clearable v-model="searchForm.fuwuzhuangtai" placeholder="请选择服务状态" >
							<el-option v-for="(item,index) in fuwuzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('dingdantixing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('dingdantixing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="fuwutupian" width="200" label="服务图片">
						<template slot-scope="scope">
							<div v-if="scope.row.fuwutupian">
								<img v-if="scope.row.fuwutupian.substring(0,4)=='http'" :src="scope.row.fuwutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.fuwutupian.split(',')[0])">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('dingdantixing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('dingdantixing','完成服务')" @click="fuwujiluCrossAddOrUpdateHandler(scope.row,'cross','','','fuwuzhuangtai','已完成','已完成,待完成'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								完成服务
							</el-button>
							<el-button class="edit" v-if=" isAuth('dingdantixing','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('dingdantixing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
...
import fuwujiluCrossAddOrUpdate from "../fuwujilu/add-or-update";
	export default {
		...
		methods: {
			...
			fuwujiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','dingdantixing');
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.fuwuzhuangtai!='' && this.searchForm.fuwuzhuangtai!=undefined){
            params['fuwuzhuangtai'] = this.searchForm.fuwuzhuangtai
          }
	...
	dingdantixingstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'dingdantixing/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
